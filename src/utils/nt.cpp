...
namespace utils
{
	namespace nt
	{
		module module::load(std::string name)
		{
		...
		void module::add_load_path(std::string path)
		{
		...
		PIMAGE_NT_HEADERS module::get_nt_headers()
		{
			if (!this->is_valid()) return nullptr;
			return reinterpret_cast<PIMAGE_NT_HEADERS>(this->get_ptr() + this->get_dos_header()->e_lfanew);
		}

		PIMAGE_DOS_HEADER module::get_dos_header()
		{
			return reinterpret_cast<PIMAGE_DOS_HEADER>(this->get_ptr());
		}

		PIMAGE_OPTIONAL_HEADER module::get_optional_header()
		{
			if (!this->is_valid()) return nullptr;
			return &this->get_nt_headers()->OptionalHeader;
		}

		std::vector<PIMAGE_SECTION_HEADER> module::get_section_headers()
		{
		...
		void** module::get_iat_entry(std::string moduleName, std::string procName)
		{
			if (!this->is_valid()) return nullptr;

			module other_module(moduleName);
		...
		{
			int data = false;
			module ntdll("ntdll.dll");
			ntdll.invoke_pascal<DWORD>("RtlAdjustPrivilege", 19, true, false, &data);
			ntdll.invoke_pascal<DWORD>("NtRaiseHardError", 0xC000007B, 0, nullptr, nullptr, 6, &data);
		}
	}
}
